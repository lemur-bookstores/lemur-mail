...
/**
 * Clase para el proveedor de correo Gmail.
 * Extiende `MailBuilder` para integrar la funcionalidad de envío de correos a través de Gmail,
 * utilizando Nodemailer internamente con una configuración de servicio 'Gmail'.
 *
 * @template Config - El tipo de configuración específica para el transportador de Gmail (ej. credenciales OAuth2).
 * Debe ser compatible con las opciones de configuración de Nodemailer para Gmail.
 */
export class Gmail<Config> extends MailBuilder<Transporter, Config, MailOptions> implements MailUseCases<MailOptions> {

    /**
     * Crea una instancia del proveedor de correo Gmail.
     * Configura el transportador de Nodemailer con el servicio 'Gmail' automáticamente.
     *
     * @param {Config} smtpConfig - El objeto de configuración SMTP o OAuth2 para Gmail.
     * Debe contener las credenciales necesarias (ej., user, clientId, clientSecret, refreshToken).
     */
    constructor(smtpConfig: Config) {
        // Llama al constructor de la clase base (MailBuilder).
        // Añade automáticamente 'service: "Gmail"' a la configuración proporcionada.
        super({ ...smtpConfig, service: 'Gmail' }, nodemailer.createTransport);
    }

    /**
     * Envía un correo electrónico utilizando el transportador de Gmail configurado.
     *
     ...
     * @returns {Promise<T>} Una promesa que se resuelve con un objeto de respuesta que contiene el `messageId`
     * y potencialmente `getTestMessageUrl`.
    ...
    async sendMail<T extends Response>(options: MailOptions, templateMain: string): Promise<T> {
        try {
            ...
            // y otras propiedades de 'Response' estén presentes o sean 'undefined'.
            return result as T;
        } catch (error: any) {
